...
var removeByIndexes_1 = require("./removeByIndexes");
var arrays_match_1 = require("@writetome51/arrays-match");
var arr = ['he', 'llo', 'bay', 'bayyyy'];
// Test 1: Make sure 0 removes first item:
removeByIndexes_1.removeByIndexes([0], arr);
if (arr[0] === 'llo')
    console.log('test 1 passed');
else
    console.log('test 1 failed');
// Test 2: Make sure -1 removes last item:
removeByIndexes_1.removeByIndexes([-1], arr);
...
arr = ['he', 'llo', 'zz'];
removeByIndexes_1.removeByIndexes([0, 2], arr);
...
removeByIndexes_1.removeByIndexes([-2, 1], arr);
...
arr = ['he', 'llo', 'zz', 'gg', 'cc', 'aa'];
removeByIndexes_1.removeByIndexes([-6, 5], arr);
...
arr = ['he', 'llo', 'zz', 'gg', 'cc', 'aa'];
removeByIndexes_1.removeByIndexes([-6, -1], arr);
if (arrays_match_1.arraysMatch(arr, ['llo', 'zz', 'gg', 'cc']))
...
try {
    removeByIndexes_1.removeByIndexes([-7], arr);
}
catch (e) {
    errorTriggered = true;
}
...
arr = ['he', 'llo', 'zz', 'gg', 'cc', 'aa'];
try {
    removeByIndexes_1.removeByIndexes([6], arr);
}
catch (e) {
    errorTriggered = true;
}
...
errorTriggered = false;
try {
    removeByIndexes_1.removeByIndexes([0], {});
}
catch (e) {
    errorTriggered = true;
}
